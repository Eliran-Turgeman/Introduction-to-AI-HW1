...
def w_experiment(maze_index):
    ...
    for w in w_values:
        ...
        was_robot = WAStartRobot(center_manhattan_heuristic, w)
        was_robot_solution = was_robot.solve(problem)
        solve_times.append(was_robot_solution.solve_time)
        solution_costs.append(was_robot_solution.cost)
        ################################################################################################################
    ...
    plt.savefig(f"plots/time_{maze_index}.png")
    plt.clf()
    ...
    plt.savefig(f"plots/cost_{maze_index}.png")
    plt.clf()
...
def shorter_robot_heuristic_experiment(maze_index):
    ...
    for k in ks:
        ...
        short_robot = WAStartRobot(heuristic=ShorterRobotHeuristic, k=k)
        short_robot_solution = short_robot.solve(problem)
        solve_times.append(short_robot_solution.solve_time)
        heuristic_init_times.append(short_robot_solution.init_heuristic_time)
        ################################################################################################################
